...
class EmployeeAnalyticsLocalDataSourceImpl
    implements EmployeeAnalyticsLocalDataSource {
  @override
  Future<EmployeeAnalyticsData> fetchOverview({
    required String timeRange,
  }) async {
    await Future.delayed(const Duration(milliseconds: 800));
    return EmployeeAnalyticsData(
      timeRange: timeRange,
      metrics: _metricsForTimeRange(timeRange),
  ...
  List<Map<String, dynamic>> _metricsForTimeRange(String timeRange) {
    return [
      {
        'title': 'Ticket Volume',
        'value': timeRange == 'Today' ? '18' : '89',
        'unit': timeRange == 'Today' ? 'today' : 'this week',
        'trend':
            timeRange == 'Today' ? '+15% vs yesterday' : '+8% vs last week',
        'isPositiveTrend': true,
      },
      {
        'title': 'Resolution Rate',
        'value': timeRange == 'Today' ? '96%' : '94%',
        'unit': 'success',
        'trend': timeRange == 'Today' ? '+2% vs yesterday' : '+5% vs last week',
        'isPositiveTrend': true,
      },
      {
      ...
      },
      {
        ...
        'value': timeRange == 'Today' ? '15' : '78',
        'unit': timeRange == 'Today' ? 'today' : 'this week',
        'trend': timeRange == 'Today' ? 'Target: 20' : 'Target: 80',
        'isPositiveTrend': timeRange == 'Today' ? false : true,
      },
      {
