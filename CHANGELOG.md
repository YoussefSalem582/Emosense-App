...
- **`lib/core/utils/assets/`** — Portfolio-style design token modules: [`AppColors`](lib/core/utils/assets/app_colors.dart) (sections, theme helpers, extra gradients & gray scale), [`AppFonts`](lib/core/utils/assets/app_fonts.dart) (Inter / Tajawal, responsive & theme-aware text styles), [`AppLocaleBinding`](lib/core/utils/assets/app_locale_binding.dart) (synced from [`MaterialApp.builder`](lib/app.dart) for static font helpers). [`constants/app_colors.dart`](lib/core/constants/app_colors.dart) re-exports the implementation so existing imports stay valid. Barrel: [`assets.dart`](lib/core/utils/assets/assets.dart); exported from [`core.dart`](lib/core/core.dart).

### Changed

- **Feature widgets (admin / employee / auth / tickets / text analysis)** — **`admin_app_bar`**: header gradient and glass chrome use **`AppColors`** ( **`primary` / `accent` / `error`**, **`white`**, **`textPrimary`** shadow ); profile avatar mini-gradient **`AppColors.error` → `warning`**. **`profile_header_widget`**: frosted layers **`AppColors.white`**; **`AppFonts`** for name, role, status chip. **`onboarding_content`**: indicator **`AppColors.white`**. **`admin_tickets_header`**: **`AppFonts`** + **`AppColors.white`** (replaces **`Theme.textTheme`** + **`Colors.white`**). **`welcome_header_widget`**: **`AppColors.surface`**, shadow from **`textPrimary`**; **`AppFonts.extraBold`** welcome line. **`divider_widget`** default **`AppColors.white`**. **`employee_tab_bar`**: elevation shadow **`textPrimary`** alpha. **`admin_ticket_card`**: surface **`AppColors.white`**; priority/status/avatar fallback **`AppColors`** semantics. **`text_input_widget`**: analyze CTA loader/icon **`AppColors.white`**.
- **Core widgets (analytics section)** — **`GoalsAndTargets`**, **`PerformanceTrendsChart`**, and **`DetailedAnalytics`** use **`AppFonts`** / **`AppColors`** (surfaces, shadows, dropdowns, legends, time metrics); removed unused **`ThemeData theme`** constructor params and updated **`analytics_section.dart`**. **`AppBarStatsRow`** stat value/label/subtitle use **`AppFonts.copyWith`** (incl. **`shadows`**). **`OnboardingDeveloperTools`** uses **`core.dart`** tokens (**`AppColors.warning`**, **`success`**, **`error`**, **`surfaceContainer`**).
- **Core widgets (cards + buttons + dev test)** — **`AnalyticsCard`**, **`DemoExamplesCard`**, **`InstructionsCard`**, **`VideoResultsCard`** (theme-aware **`AppFonts`**, **`AppColors`** borders/surfaces), **`SystemMetricsCard`**, **`ResultsCard`** (incl. gradient hero typography), **`ModernButton`** / **`GlassButton`** label (**`AppFonts.button`** + dynamic size; drops hardcoded Inter), and **`BackendConnectionTest`** (**`AppFonts`**, **`AppColors`** semantic surfaces/borders) replace remaining ad hoc **`TextStyle`** / raw **`Colors.*`** / amber/indigo where touched; **`demo_examples_card`** demo chrome uses **`AppColors.primaryDark`** / **`info`** instead of Material indigo/blue.
- **Core widgets & routing** — **`AppRouter`** `_NotFoundScreen`, **`ScreenStateManager`** snackbar label/icon colors, **`AnalysisAppBar`** (titles, badge, back/menu tints), **`AnalyticsHeader`** dropdown, and **`EmotionInputField`** use **`AppFonts`** / **`AppColors`** instead of ad hoc **`TextStyle`** / **`Colors.*`** / **`0xFF667EEA`**. **`screen_state_manager`** now imports **`../core.dart`** (replaces **`constants/app_colors.dart`** only).
- **Typography rollout** — **`lib/features/emotion/presentation/pages/analytics/`** (`customer_analytics_screen`, `team_performance_screen`, `live_monitor_screen`): replaced remaining **`TextStyle`** / **`const TextStyle`** with **`AppFonts`** + **`AppColors`** semantic tokens (body/heading/caption/label, **`AppFonts.copyWith`** for weights and sizes).
- **Typography rollout** — Shell pages under `lib/features/shared/presentation/pages/`, all auth presentation widgets under `lib/features/auth/`, shared analysis widgets under `lib/features/analysis/shared/presentation/widgets/`, **tickets** (`employee`/`admin` presentation widgets, shared `create_ticket_dialog`, `employee_tickets_page`), and **`admin_profile_screen`** now use **`AppFonts`** (via `package:emosense_mobile/core/core.dart`) instead of ad hoc `TextStyle`s where touched; auth/splash gradients and buttons align with **`AppColors.primary` / `accent`**. **`AppFonts.copyWith`** now forwards **`shadows`** and **`decorationColor`**; **`AppFonts.h6ForTheme`** added for section titles. **Emotion analytics** screens (`statistics`, `live_monitor`, `team_performance`, `customer_analytics`) use **`AppColors`** for frosted chrome / tab bars / shadows (**`AppColors.white`**, **`AppColors.surface`**, **`AppColors.textPrimary`** shadow) instead of raw **`Colors.white`** / **`Colors.black`**. **`employee_dialogs`** uses **`AppColors.primary`** / **`AppColors.border`** / **`AppColors.warning`** instead of indigo hex and **`Colors.grey`/`amber`**.
- **Typography rollout (employee / admin)** — Remaining screens under `lib/features/employee` (profile widgets, bottom nav, performance), **`lib/features/admin`** (dashboard, navigation, dialogs, system config, user management, profile, app bar, bottom nav) now use **`AppFonts`** + **`AppColors`** tokens instead of inline **`TextStyle`** / raw **`Colors.*`** where edited.
